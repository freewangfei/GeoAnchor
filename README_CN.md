...
GeoAnchor-Net 是一个面向地震孔隙度预测的监督感知条件框架，可同时适用于密集基准监督和稀疏现场井控制场景。它使用同一个模型处理密集标签全场重建和稀疏井盲区间预测。

其关键设计是**绝对锚点传播**：将稀疏井观测作为孔隙度值传播，而不是作为残差修正传播。这样可以在强低频先验主导预测空间时，将直接井证据保持在物理属性尺度上。GeoAnchor-Net 结合趋势构建、结构条件修正和锚点一致细化，在不同监督机制下生成孔隙度预测。

在已发布的合成基准、公共 F3 现场数据接口和 Seis2Rock 派生迁移划分上的实验中，GeoAnchor-Net 分别取得 `0.01938`、`0.01462`、`0.02684` 和 `0.02494` 的 RMSE。在 `OpenPoroBench-S` 和 `F3 Demo 2023` 上，相对于最强复现比较方法，GeoAnchor-Net 的 RMSE 分别降低 `20.87%` 和 `40.76%`。

本仓库包含 GeoAnchor-Net 的完整训练与评价代码。
